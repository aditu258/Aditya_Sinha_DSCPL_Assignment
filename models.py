...
from video_agent import VideoContentAgent
...
# Initialize video content agent
video_agent = VideoContentAgent()
...
class StateManager:
    ...
    def get_program_history(user_id: str) -> List[Dict[str, Any]]:
        ...
        for row in cursor.fetchall():
            # Get actual progress for this program
            progress = StateManager.get_program_progress(row[1])
            all_days_completed = len(progress["completed_days"]) == progress["total_days"]
            ...
            history.append({
                ...
                "start_date": row[5],
                "end_date": row[6],
                "completed": all_days_completed,  # Only mark as completed if all days are done
                "paused": bool(row[8])
            })
    ...
    def get_program_progress(session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Safely get program length and current day with defaults
            program_length = session.get("program_length")
            current_day = session.get("current_day")
            
            # Convert to integers with proper error handling
            try:
                program_length = int(program_length) if program_length is not None else 0
                current_day = int(current_day) if current_day is not None else 0
            ...
            for day in range(1, program_length + 1):
            ...
            print(f"Error getting program progress: {str(e)}")
            return {
                "total_days": 0,
                "current_day": 0,
                "completed_days": [],
...
# Content generators
def generate_devotion_content(topic: str, day: int, content_type: ContentType = ContentType.BOTH):
    """Generate devotion content including scripture, prayer, and video recommendations."""
    # Generate scripture and prayer content
    content = llm.invoke(f"""Create a daily devotion about {topic} including:
1. Scripture passage with reference
2. Brief explanation
3. Prayer
4. Declaration of faith""")
    
    # Get video recommendations if content type includes video
    video_recommendation = None
    if content_type in [ContentType.VIDEO, ContentType.BOTH]:
        videos = video_agent.get_recommendations("Daily Devotion", topic)
        video_recommendation = video_agent.format_recommendations(videos, topic, "Daily Devotion")
    
    # Parse the content into sections
    sections = str(content.content).split("\n\n")
    scripture = sections[0] if len(sections) > 0 else ""
    prayer = sections[1] if len(sections) > 1 else ""
    declaration = sections[2] if len(sections) > 2 else ""
...
def generate_prayer_content(topic: str):
    prayer = llm.invoke(f"Create an ACTS model prayer about {topic}")
    return prayer.content

def generate_meditation_content(topic: str):
    meditation = llm.invoke(f"""Create a meditation guide about {topic} including:
1. Scripture focus
2. Meditation prompts
3. Breathing guide""")
    return meditation.content

def generate_accountability_content(topic: str):
    content = llm.invoke(f"""Create accountability support for {topic} including:
1. Scripture
